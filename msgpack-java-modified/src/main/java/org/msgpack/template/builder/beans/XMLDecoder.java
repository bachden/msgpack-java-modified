	...
	private class SAXHandler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName, Attributes attributes)
			...
			if (!inJavaElem) {
				if ("java".equals(qName)) {
					...
					listener.exceptionThrown(new Exception(Messages.getString("custom.beans.72", qName)));
				}
				return;
			}
		...
		private Class<?> classForName(String className) throws ClassNotFoundException {
			if ("boolean".equals(className)) {
				...
				return Class.forName(className, true, defaultClassLoader == null
						? Thread.currentThread().getContextClassLoader() : defaultClassLoader);
			}
		}
		...
		private Object executeCommon(Elem elem) throws Exception {
			...
			if (elem.target == owner) {
				...
				Method mostSpecificMethod = findMethod(owner instanceof Class<?> ? (Class<?>) owner : owner.getClass(),
						method, c);
				...
				if (value == null && elem.attributes != null) {
					...
					if (codeAttr != null) {
						Character character = new Character(
								(char) Integer.valueOf(codeAttr.substring(1), 16).intValue());
