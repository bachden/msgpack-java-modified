...
public class BeansBuildContext extends BuildContext<BeansFieldEntry> {
	...
	protected String buildReadMethodBody() {
		...
		for (int i = 0; i < entries.length; i++) {
			...
			if (type.isPrimitive()) {
				...
				buildString("_$$_t.%s( (%s)this.templates[%d].read($1, _$$_t.%s()) );", e.getSetterName(),
						e.getJavaTypeName(), i, e.getGetterName());
			}
	...
	@Override
	public void writeTemplate(Class<?> targetClass, BeansFieldEntry[] entries, Template[] templates,
