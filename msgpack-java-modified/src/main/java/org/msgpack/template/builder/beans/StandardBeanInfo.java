...
class StandardBeanInfo extends SimpleBeanInfo {
	...
	/*
	 * MODIFIED FOR THE MSGPACK PROJECT merge the PropertyDescriptor with
	 * superclass
	 */
						...
						if ((subGet == null || subSet == null) && (superGet != null)) {
							...
							if (subGet != null) {
								...
								for (MethodDescriptor methodDesc : introspectMethods) {
									...
									if (method != subGet && subGetName.equals(method.getName())
	...
	/*
	 * MODIFIED FOR THE MSGPACK PROJECT merge the MethodDescriptor
	 */
	...
	/*
	 * MODIFIED FOR THE MSGPACK PROJECT find the add, remove listener method
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private static void introspectListenerMethods(String type, Method theMethod,
	...
	private static void introspectGetListenerMethods(Method theMethod, HashMap<String, HashMap> methodsTable) {
		...
		if ((returnType.getComponentType() == null)
				|| (!returnType.getComponentType().getName().endsWith(listenerName))) {
			return;
		}
	...
	void init() {
		...
		if (properties != null) {
			String defaultPropertyName = (defaultPropertyIndex != -1 ? properties[defaultPropertyIndex].getName()
					: null);
