...
public class PropertyDescriptor extends FeatureDescriptor {
	...
	public int hashCode() {
		return BeansUtils.getHashCode(getter) + BeansUtils.getHashCode(setter)
				+ BeansUtils.getHashCode(getPropertyType()) + BeansUtils.getHashCode(getPropertyEditorClass())
				+ BeansUtils.getHashCode(isBound()) + BeansUtils.getHashCode(isConstrained());
	}
	...
	void setWriteMethod(Class<?> beanClass, String setterName) throws IntrospectionException {
		...
		if (writeMethod == null) {
			throw new IntrospectionException(Messages.getString("custom.beans.64", setterName)); //$NON-NLS-1$
		}
	...
	public PropertyEditor createPropertyEditor(Object bean) {
		...
		try {
			...
			RuntimeException re = new RuntimeException(Messages.getString("custom.beans.47"), e); //$NON-NLS-1$
			throw re;
		}
		return editor;
	}
