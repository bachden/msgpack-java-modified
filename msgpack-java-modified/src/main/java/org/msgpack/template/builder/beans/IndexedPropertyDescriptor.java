...
public class IndexedPropertyDescriptor extends PropertyDescriptor {
	...
	private void setIndexedByName(Class<?> beanClass, String indexedGetterName, String indexedSetterName)
		...
		if (!isCompatible()) {
			// custom.beans.57=Property type is incompatible with the indexed
			// property type
	...
	public boolean equals(Object obj) {
		...
		return (super.equals(other)
				...
				&& (indexedSetter == null ? other.indexedSetter == null : indexedSetter.equals(other.indexedSetter)));
	}
	...
	private void internalSetIndexedReadMethod(Method indexGetter) throws IntrospectionException {
		...
		if (indexGetter == null) {
			if (indexedSetter == null) {
				if (getPropertyType() != null) {
					// custom.beans.5A=Indexed method is not compatible with non
					// indexed method
		...
		if ((indexGetter.getParameterTypes().length != 1) || (indexGetter.getParameterTypes()[0] != Integer.TYPE)) {
			// custom.beans.5B=Indexed read method must take a single int
			// argument
		...
		if (indexedReadType == Void.TYPE) {
			...
			// custom.beans.5A=Indexed read method is not compatible with
	...
	private void setIndexedWriteMethod(Class<?> beanClass, String indexedSetterName) throws IntrospectionException {
		Method setter = null;
		try {
			setter = beanClass.getMethod(indexedSetterName,
					new Class[] { Integer.TYPE, getPropertyType().getComponentType() });
	...
	private void internalSetIndexedWriteMethod(Method indexSetter, boolean initialize) throws IntrospectionException {
		...
		if (indexSetter == null) {
			if (indexedGetter == null) {
				if (getPropertyType() != null) {
					// custom.beans.5E=Indexed method is not compatible with non
					// indexed method
					throw new IntrospectionException(Messages.getString("custom.beans.5E")); //$NON-NLS-1$
				}
				indexedPropertyType = null;
			}
		...
		if (indexedSetterArgs[0] != Integer.TYPE) {
			// custom.beans.60=Indexed write method must take an int as its
			// first argument
		...
		if (initialize && indexedGetter == null) {
			...
			if (indexedPropertyType != indexedWriteType) {
				// custom.beans.61=Indexed write method is not compatible with
				// indexed read method
